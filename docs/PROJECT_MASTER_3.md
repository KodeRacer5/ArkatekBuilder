...
│   │   │   ├── AnalyticalChain.jsx      # Banner canvas — Meto nodes + 4-node dose calculator pipeline (112 protocols)
│   │   │   ├── chat-input.jsx
...
│   ├── PROTOCOL_REFERENCE.md            # 112-compound dosing reference — verified + PRECLINICAL flags
│   └── HANDOFF_*.md
...
- `AnalyticalChain` overlay — interactive banner canvas with two systems:
  - **Meto Pipeline** — live triad scores from global store (5 input nodes → engine → 5 triad nodes → ORI)
  - **Dose Calculator Pipeline** — 4-node pipeline: Protocol Lookup → Vial Config → Dose Calculator → Injection Schedule
  - 112 protocols searchable in dropdown, PRECLINICAL flag on unverified compounds
  - Nodes draggable, positions persisted via localStorage, multi-select via box-drag, reset button restores default positions + viewport
...
### Protocols — `/protocols`
- Sidebar: section headers (collapsible), plain scroll list, selected state highlight, PRECLINICAL flag on unverified compounds
- Right panel: CardSwap 3D flip cards — Overview, Dosing, Administration, Stack Notes per protocol
- 140+ protocols across 20 sections including: Signaling Peptides, GH Secretagogues, Bioregulators, Immune, GLP-1/Weight Loss, Fat Loss/Metabolic, Testosterone/TRT, Anabolic Compounds, PCT, Reproductive/Hormonal, Skin/Aesthetic, Neuroprotection/Cognitive, Immune/Thymic, Longevity (Additional), Research Pipeline
- Layout LOCKED — no AnimatedList, no layout changes
...
| AnalyticalChain localStorage key | `pj_banner_node_positions` — node positions; `pj_dose_pipeline_state` — selected protocol |
| AnalyticalChain node extent | `[[0,0],[1800,300]]` — nodes cannot leave banner bounds |
| AnalyticalChain multi-select | Box-drag on empty canvas. Ctrl+click to add to selection |
| Protocol PRECLINICAL flag | `exp:true` on protocol object — shown in dropdown + VialConfigNode header |
| Protocol reference doc | `docs/PROTOCOL_REFERENCE.md` — 112 compounds, verified dosing, safety flags |

### Protocols Page Layout — LOCKED
- Sidebar: section headers (collapsible), plain scroll list, selected state highlight
- PRECLINICAL label shown next to compound name in sidebar and category header in right panel
- No AnimatedList component on protocols page — custom scroll list
- CardSwap on right panel — 4 cards: Overview, Dosing, Administration, Stack Notes
- 140+ protocols, 20 sections — do not remove or restructure existing sections
...
- [x] Protocols page — sidebar (LOCKED layout) + CardSwap (140+ protocols, 20 categories)
...
- [x] AnalyticalChain — dual canvas system: Meto pipeline + 4-node dose calculator
- [x] AnalyticalChain — 112 protocols, node positions persisted via localStorage, multi-select, reset button
- [x] AnalyticalChain — PRECLINICAL flag on 20 unverified compounds in dropdown + vial config node
- [x] Banner system (hero-banner4.png, full-width, per-page)
- [x] Banner canvas pointer-events fix — TopBar clicks pass through, nodes remain draggable
...
- [x] PROTOCOL_REFERENCE.md — 112-compound verified dosing reference with safety flags
...
- [ ] AnalyticalChain node styling — restyle dose pipeline nodes to match Meto node aesthetic
