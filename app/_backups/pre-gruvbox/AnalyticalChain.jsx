...
function MiniTriadNode({ data, onEdit }) {
  ...
  return (
    <div onClick={()=>s&&onEdit&&onEdit(data)}
      style={{background:s?`${c}14`:'var(--gb-bg-soft)',border:`1px solid ${s?c+'66':'var(--gb-bg-2)'}`,borderRadius:8,minWidth:100,overflow:'hidden',transition:'all 0.4s',cursor:s?'pointer':'default'}}>
      ...
      <div style={{padding:'4px 8px',display:'flex',alignItems:'center',gap:6}}>
        ...
        {s&&data.composite_score!=null&&<div style={{fontSize:11,fontWeight:800,color:rc||c,lineHeight:1}}>{Math.round(data.composite_score)}</div>}
      </div>
    </div>
  )
}
...
function buildMetoGraph(results, onEdit) {
  const nodes=[], edges=[]
  const INPUTS = [
  ...
  INPUTS.forEach((inp,i)=>{
    nodes.push({id:`in_${inp.id}`,type:'mini_input',position:{x:0,y:i*48},data:{...inp,filled:!!results}})
  ...
  nodes.push({id:'engine',type:'mini_engine',position:{x:160,y:80},data:{active:!!results,mode:results?.mode}})
  TRIADS.forEach((t,i)=>{
    const triad=results?.triads.find(tr=>tr.id===t.id), scored=!!triad
    nodes.push({id:`tr_${t.id}`,type:'mini_triad',position:{x:330,y:i*52-20},data:{...t,scored,onEdit,composite_score:triad?.scores.composite_score??null,risk_level:triad?.scores.risk_level??null}})
  ...
  nodes.push({id:'overall',type:'mini_overall',position:{x:500,y:80},data:{scored:!!results,overall_risk_index:results?.overall.overall_risk_index??null,mode:results?.mode}})
  return {nodes,edges}
}
...
function BannerCanvasInner({ height }) {
  ...
  useEffect(()=>{
    const {nodes:n,edges:e}=buildMetoGraph(results,handleEdit)
    setNodes(n);setEdges(e)
  },[results,handleEdit])

  useEffect(()=>{ setTimeout(()=>setViewport({x:10,y:10,zoom:0.9}),50) },[])

  return (
    <div style={{width:'100%',height,position:'relative'}}>
      <style>{`
        .banner-canvas .react-flow__attribution{display:none}
        .banner-canvas .react-flow__node{background:transparent!important;border:none!important;box-shadow:none!important;padding:0!important}
