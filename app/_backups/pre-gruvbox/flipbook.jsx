...
const Flipbook = memo(({ viewerStates, setViewerStates, flipbookRef, pdfDetails, pagesPath }) => {
    const { ref, width, height, refreshSize } = useRefSize();
    const [scale, setScale] = useState(1);
    const [wrapperCss, setWrapperCss] = useState({});
    const [viewRange, setViewRange] = useState([0, 12]);
    ...
    return (
        <div ref={ref} className={cn("relative flex-1 w-full bg-transparent flex justify-center items-center overflow-hidden", screenfull?.isFullscreen ? 'h-[calc(100vh-4rem)]' : 'h-[calc(100vh-8rem)]')}>
