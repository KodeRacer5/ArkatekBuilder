...
const FlipbookLoader = forwardRef(({ pdfDetails, scale, viewerStates, setViewerStates, viewRange, setViewRange, pagesPath }, ref) => {
    ...
    const onFlip = useCallback((e) => {
        ...
        if (e.data > viewerStates.currentPageIndex) {
            newViewRange = [viewRange[0], Math.max(Math.min(e.data + 6, pdfDetails.totalPages), viewRange[1])]
        } else if (e.data < viewerStates.currentPageIndex) {
            newViewRange = [Math.min(Math.max(e.data - 6, 0), viewRange[0]), viewRange[1]]
        } else {
    ...
    return (
        <div className="relative" style={{
            filter: 'drop-shadow(0 24px 48px rgba(0,0,0,0.8)) drop-shadow(0 8px 16px rgba(0,0,0,0.5))',
        }}>
            <HTMLFlipBook
                ref={ref}
                key={scale}
            ...
            >
                {
                    Array.from({ length: pdfDetails.totalPages }, (_, index) => (
                        <MemoizedPdfPage
                            ...
                            pagesPath={pagesPath}
                            isHard={index === 0 || index === pdfDetails.totalPages - 1}
                        />
                    ))
                }
            </HTMLFlipBook>
