...
const AnimatedItem = ({ children, delay = 0, index, onMouseEnter, onClick }) => {
  ...
  return (
    <motion.div
      ...
      transition={hovered
        ? { type: 'spring', stiffness: 500, damping: 20, mass: 0.08 }
        : { type: 'spring', stiffness: 300, damping: 22, mass: 0.1, delay }
      }
      style={{ marginBottom: '0.5rem', cursor: 'pointer', transformOrigin: 'left center' }}
    >
      {children}
    </motion.div>
  );
...
const AnimatedList = ({ items = [], onItemSelect, showGradients = true,
  enableArrowNavigation = true, className = '', itemClassName = '',
  displayScrollbar = false, initialSelectedIndex = -1 }) => {
        ...
        {items.map((item, index) => (
          <AnimatedItem key={index} delay={0.05} index={index}
            ...
            <div className={`item ${selectedIndex === index ? 'selected' : ''} ${itemClassName}`}>
              <p className="item-text">{item.label || item}</p>
            </div>
          </AnimatedItem>
        ))}
      </div>
