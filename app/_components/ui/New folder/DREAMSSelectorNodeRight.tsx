...
export interface DREAMSSelectorRightData {
...
// ── Orientation locked: hub LEFT, pills extend RIGHT ──────────────────────────
const ORIENTATION = 'right' as const;
...
const R = {
  hubX:      0.199, // hub center X — left side
  hubY:      0.500,
  hubR:      0.207,
  innerRing: 0.247,
  outerRing: 0.443,
...
function computeGeo(w: number, h: number) {
  ...
  // Connector exits LEFT edge of pill (facing hub)
  const connX = (g: PillGeo) => g.px;

  return { HX, HY, HR, innerR, outerR, PW, PH, PR, pills, connX };
}

// ── Animations (injected once) ────────────────────────────────────────────────
const ANIM_ID = 'dsnr-keyframes';
const ANIMATIONS = `
  @keyframes dsnr-hub-in  { from{opacity:0;transform:scale(0.5)} to{opacity:1;transform:scale(1)} }
  @keyframes dsnr-ring-in { from{opacity:0;transform:scale(0.6) rotate(-20deg)} to{opacity:1;transform:scale(1) rotate(0deg)} }
  @keyframes dsnr-pill-in { from{opacity:0;transform:translateX(40px) scaleX(0.7)} to{opacity:1;transform:translateX(0) scaleX(1)} }
  @keyframes dsnr-line-in { from{opacity:0;stroke-dashoffset:60} to{opacity:0.85;stroke-dashoffset:0} }
`;
...
const DREAMSSelectorNodeRight: React.FC<NodeProps & { data: DREAMSSelectorRightData }> = ({
  data,
  width  = DEFAULT_WIDTH,
  ...
  const uid = useMemo(() => `dsnr-${Math.random().toString(36).slice(2, 7)}`, []);
    ...
    <div className="nopan nodrag" style={{ width: '100%', height: '100%', position: 'relative', background: 'transparent' }}>
      ...
      <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`}
        ...
        <circle cx={HX} cy={HY} r={outerR} fill="none"
          ...
          style={{ transformOrigin: `${HX}px ${HY}px`, animation: mounted ? 'dsnr-ring-in 0.8s cubic-bezier(0.34,1.2,0.64,1) 0.05s both' : 'none', opacity: mounted ? undefined : 0 }} />
        ...
        {/* Hub */}
        <g style={{ transformOrigin: `${HX}px ${HY}px`, animation: mounted ? 'dsnr-hub-in 0.65s cubic-bezier(0.34,1.56,0.64,1) 0.1s both' : 'none', opacity: mounted ? undefined : 0 }}>
          ...
          return (
            <g key={v.id}>
              <line x1={g.nx} y1={g.ny} x2={connX(g)} y2={g.pcy}
                ...
                style={{ animation: mounted ? `dsnr-line-in 0.4s ease-out ${0.25 + i * 0.07}s both` : 'none', opacity: mounted ? undefined : 0, transition: 'stroke-width 0.2s' }} />
              ...
              <g filter={isH ? `url(#${uid}-glow${i})` : `url(#${uid}-soft)`}
                style={{ opacity: mounted ? (opaque ? 1 : 0.32) : 0, cursor: opaque ? 'pointer' : 'default',
                  animation: mounted ? `dsnr-pill-in 0.55s cubic-bezier(0.34,1.4,0.64,1) ${0.20 + i * 0.07}s both` : 'none',
                ...
                {isQ && <circle cx={g.px + PW - PR * 0.6} cy={g.pcy} r={fs.dotR} fill="white" opacity={0.9} />}
              </g>
            </g>
          );
        })}
...
export default DREAMSSelectorNodeRight;

export const DREAMS_RIGHT_NODE_TYPES = { dreamsSelectorRight: DREAMSSelectorNodeRight };
