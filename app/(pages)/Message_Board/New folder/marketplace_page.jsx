    ...
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{height:120,background:`linear-gradient(145deg,rgba(60,56,54,0.8),rgba(40,38,36,0.9))`,display:'flex',alignItems:'center',justifyContent:'center',borderBottom:`1px solid ${T.border}`,position:'relative',flexShrink:0}}>
        <div style={{width:60,height:60,borderRadius:16,background:`${product.badgeC}14`,border:`1px solid ${product.badgeC}30`,display:'flex',alignItems:'center',justifyContent:'center',boxShadow:`0 0 20px ${product.badgeC}18`}}>
          ...
          {product.cat==='nutraceuticals'&&(
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={product.badgeC} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M9 2v6"/><path d="M15 2v6"/><path d="M7 8h10l1 12H6L7 8z"/><path d="M9 12h6"/>
            </svg>
          )}
          {product.cat==='lab-kits'&&(
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={product.badgeC} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          {items.length===0 ? (
            ...
            <div key={item.id} style={{display:'flex',gap:12,padding:'12px 0',borderBottom:`1px solid ${T.border}`,alignItems:'center'}}>
              <div style={{width:44,height:44,borderRadius:10,background:`${item.badgeC}14`,border:`1px solid ${item.badgeC}25`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                {item.cat==='peptides'&&<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={item.badgeC} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3H5a2 2 0 00-2 2v3m18 0V5a2 2 0 00-2-2h-3m0 18h3a2 2 0 002-2v-3M3 16v3a2 2 0 002 2h3"/></svg>}
                {item.cat==='nutraceuticals'&&<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={item.badgeC} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 2v6"/><path d="M15 2v6"/><path d="M7 8h10l1 12H6L7 8z"/></svg>}
                {item.cat==='lab-kits'&&<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={item.badgeC} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>}
                {item.cat==='devices'&&<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={item.badgeC} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>}
              </div>
    ...
    <div style={{width:'100vw',height:'100vh',background:T.bg,display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system,sans-serif'}}>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden',minHeight:0}}>
        ...
        <div style={{width:240,flexShrink:0,background:T.sidebar,borderRight:`1px solid ${T.border}`,display:'flex',flexDirection:'column',boxShadow:'4px 0 20px rgba(0,0,0,0.4)'}}>
          {/* Store brand — icon navigates back to Message Board */}
          <div style={{padding:'14px 16px',borderBottom:`1px solid ${T.border}`,background:'rgba(0,0,0,0.2)',flexShrink:0,display:'flex',alignItems:'center',gap:12,cursor:'pointer'}}
            onClick={()=>window.location.href='/Message_Board'} title="Back to CortixHealth">
            {/* CortixHealth icon */}
            <div style={{width:38,height:38,borderRadius:10,overflow:'hidden',flexShrink:0,
              boxShadow:'0 3px 12px rgba(75,171,19,0.35),inset 0 1px 0 rgba(255,255,255,0.15)'}}>
              <img src="/icons/CortixHealth_icon.svg" alt="CortixHealth" style={{width:'100%',height:'100%',display:'block',objectFit:'cover'}}/>
            </div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:14,fontWeight:800,color:T.text,letterSpacing:'.04em'}}>CortixMarket</div>
              <div style={{fontSize:10,color:T.textLow,marginTop:1,display:'flex',alignItems:'center',gap:4}}>
