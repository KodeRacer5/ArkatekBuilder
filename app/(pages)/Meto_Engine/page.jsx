...
function MetaConfigNode({ data }) {
  ...
  return (
    <div style={{background:a?`${c}18`:'linear-gradient(145deg,rgba(44,42,40,0.95) 0%,rgba(26,24,22,0.98) 100%)',border:`1.5px solid ${a?c+'99':c+'33'}`,borderRadius:18,minWidth:200,padding:'12px 16px',boxShadow:a?`0 0 24px ${c}22,0 4px 16px rgba(0,0,0,0.5)`:'0 8px 28px rgba(0,0,0,0.55),inset 0 1px 0 rgba(235,219,178,0.06)',transition:'all 0.4s ease'}}>
...
function MetaInputNode({ data }) {
  ...
  return (
    <div style={{background:f?`${c}18`:'linear-gradient(145deg,rgba(44,42,40,0.95) 0%,rgba(26,24,22,0.98) 100%)',border:`1.5px solid ${f?c:c+'44'}`,borderRadius:16,padding:'9px 14px',minWidth:160,boxShadow:f?`0 0 18px ${c}30,0 4px 14px rgba(0,0,0,0.5)`:'0 8px 28px rgba(0,0,0,0.55),inset 0 1px 0 rgba(235,219,178,0.06)',transition:'all 0.4s ease'}}>
...
function MetaEngineNode({ data }) {
  ...
  return (
    <div style={{background:a?'rgba(255,255,255,0.06)':'linear-gradient(145deg,rgba(44,42,40,0.95) 0%,rgba(26,24,22,0.98) 100%)',border:`2px solid ${a?'rgba(255,255,255,0.7)':'rgba(255,255,255,0.2)'}`,borderRadius:20,minWidth:200,padding:'16px 20px',boxShadow:a?'0 0 40px rgba(255,255,255,0.12),0 0 80px rgba(255,255,255,0.08)':'0 8px 32px rgba(0,0,0,0.6),inset 0 1px 0 rgba(235,219,178,0.06)',transition:'all 0.5s ease',textAlign:'center'}}>
...
function MetaFlagNode({ data }) {
  ...
  return (
    <div style={{background:hasFlags?`${c}12`:'linear-gradient(145deg,rgba(44,42,40,0.95) 0%,rgba(26,24,22,0.98) 100%)',border:`1.5px solid ${hasFlags?c+'77':c+'33'}`,borderRadius:16,minWidth:185,padding:'11px 15px',boxShadow:hasFlags?`0 0 18px ${c}20,0 3px 12px rgba(0,0,0,0.4)`:'0 8px 24px rgba(0,0,0,0.5),inset 0 1px 0 rgba(235,219,178,0.05)',transition:'all 0.4s ease'}}>
...
function MetaScoreNode({ data }) {
  ...
  return (
    <div style={{background:s?`${c}14`:'linear-gradient(145deg,rgba(44,42,40,0.95) 0%,rgba(26,24,22,0.98) 100%)',border:`1.5px solid ${s?c+'88':c+'33'}`,borderRadius:18,minWidth:180,boxShadow:s?`0 0 24px ${c}22,0 4px 16px rgba(0,0,0,0.5)`:'0 4px 14px rgba(0,0,0,0.4)',transition:'all 0.5s ease',overflow:'hidden'}}>
...
function MetaResultNode({ data }) {
  ...
  return (
    <div style={{background:r?`${c}12`:'linear-gradient(145deg,rgba(44,42,40,0.95) 0%,rgba(26,24,22,0.98) 100%)',border:`2px solid ${r?c+'77':c+'33'}`,borderRadius:20,minWidth:210,boxShadow:r?`0 0 32px ${c}18,0 4px 20px rgba(0,0,0,0.5)`:'0 4px 14px rgba(0,0,0,0.4)',transition:'all 0.5s ease'}}>
...
function MetaStorageNode({ data }) {
  ...
  return (
    <div style={{background:s?`${c}14`:'linear-gradient(145deg,rgba(44,42,40,0.95) 0%,rgba(26,24,22,0.98) 100%)',border:`1.5px solid ${s?c+'88':c+'33'}`,borderRadius:16,minWidth:185,padding:'11px 15px',boxShadow:s?`0 0 20px ${c}18,0 4px 14px rgba(0,0,0,0.4)`:'0 8px 24px rgba(0,0,0,0.5),inset 0 1px 0 rgba(235,219,178,0.05)',transition:'all 0.5s ease'}}>
...
// ─────────────────────────────────────────────────────────────────────────────
const iS = {background:'rgba(255,255,255,0.06)',border:'1px solid rgba(235,219,178,0.12)',borderRadius:8,color:'#ebdbb2',fontSize:12,padding:'6px 10px',width:'100%',outline:'none',boxShadow:'inset 0 2px 6px rgba(0,0,0,0.3)'}
...
function MetoEngineContent() {
  ...
  const { setResults: setStoreResults } = useMetoStore()

  const handleSubmit = () => {
    const r = runScoringEngine(form)
    setResults(r)
    setStoreResults(r, form)
  ...
  return (
    <div style={{width:'100vw',height:'100vh',background:'var(--canvas-bg, #0c0e14)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden',minHeight:0}}>
        ...
        {/* ── Intake panel ── */}
        <div style={{width:320,flexShrink:0,borderRight:'1px solid rgba(255,255,255,0.08)',display:'flex',flexDirection:'column',background:'linear-gradient(180deg,rgba(28,26,24,0.98) 0%,rgba(18,16,14,0.99) 100%)',backdropFilter:'blur(20px)',WebkitBackdropFilter:'blur(20px)',boxShadow:'4px 0 24px rgba(0,0,0,0.5),inset -1px 0 0 rgba(235,219,178,0.06)'}}>

          {/* Section nav */}
          <div style={{padding:'12px 10px 8px',borderBottom:'1px solid rgba(235,219,178,0.08)',display:'flex',flexDirection:'column',gap:3,background:'rgba(0,0,0,0.2)'}}>
            {SECTIONS.map(sec=>{
              const isFilled = filled.includes(sec)
              const isActive = activeSection===sec
              return (
                <button key={sec} onClick={()=>setActiveSection(sec)}
                  style={{background:isActive?'rgba(235,219,178,0.1)':'transparent',border:isActive?'1px solid rgba(235,219,178,0.2)':'1px solid transparent',cursor:'pointer',padding:'7px 10px',borderRadius:8,fontSize:10,fontWeight:700,letterSpacing:'0.12em',textTransform:'uppercase',textAlign:'left',width:'100%',color:isActive?'#ebdbb2':isFilled?'rgba(235,219,178,0.55)':'rgba(255,255,255,0.25)',transition:'all 0.2s',boxShadow:isActive?'0 2px 8px rgba(0,0,0,0.3)':'none'}}>
          ...
          <div style={{padding:'10px 14px',borderTop:'1px solid rgba(255,255,255,0.08)'}}>
            <button onClick={handleSubmit}
              style={{width:'100%',padding:'11px 0',borderRadius:10,border:'1px solid rgba(250,189,47,0.4)',background:'linear-gradient(135deg,rgba(250,189,47,0.18) 0%,rgba(254,128,25,0.12) 100%)',color:'#fabd2f',fontSize:12,fontWeight:700,letterSpacing:'0.15em',textTransform:'uppercase',cursor:'pointer',transition:'all 0.2s',boxShadow:'0 4px 16px rgba(250,189,47,0.15),inset 0 1px 0 rgba(250,189,47,0.2)'}}
        ...
        {/* ── Canvas ── */}
        <div style={{flex:1,position:'relative',height:'100%',width:'100%',minHeight:0}}>
          <ReactFlow
            nodes={nodes} edges={edges}
            onNodesChange={onNodesChange} onEdgesChange={onEdgesChange}
            nodeTypes={nodeTypes}
            fitView fitViewOptions={{padding:0.18,maxZoom:0.85}}
            style={{background:'var(--canvas-bg, #0c0e14)',width:'100%',height:'100%'}}
            minZoom={0.15} maxZoom={2}
            proOptions={{hideAttribution:true}}>
            <Background color="rgba(255,255,255,0.08)" gap={28} size={3}/>
            <Controls style={{background:'var(--gb-bg-1)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:6}}/>
          </ReactFlow>

          {/* Pipeline legend */}
          <div style={{position:'absolute',top:12,right:12,background:'linear-gradient(145deg,rgba(44,42,40,0.95),rgba(22,20,18,0.98))',border:'1px solid rgba(235,219,178,0.1)',borderRadius:14,padding:'12px 14px',backdropFilter:'blur(20px)',boxShadow:'0 8px 32px rgba(0,0,0,0.6),inset 0 1px 0 rgba(235,219,178,0.08)'}}>
